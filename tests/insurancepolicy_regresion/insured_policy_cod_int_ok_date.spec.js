...
test('Insured policy cod_int OK Date', async ({ request }) => {
  const requestUrl = '/workshop/v1/insurance_policies-photos/search?thirdPartyInternalId=330749452011&date=2025-12-31T00:00:00.000Z&pageNumber=1&pageSize=10';
  const headers = {
  ...
  const t0 = Date.now();
  const response = await request.get(`/workshop/v1/insurance_policies-photos/search?thirdPartyInternalId=330749452011&date=2025-12-31T00:00:00.000Z&pageNumber=1&pageSize=10`, { headers });
...
await test.step("Items array length does not exceed pageSize", async () => {
    const jsonData = await response.json();
    if (jsonData.items!=null){expect(jsonData.items.length).to.be.at.most(jsonData.pageSize);}
});
